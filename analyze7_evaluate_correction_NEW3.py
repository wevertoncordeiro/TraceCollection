...
if __name__ == '__main__':
	...
	print
	"\tlog_file_name:", out_log_file_name
	print
	"\tomit-omit_pif:", omit_pif
	print
	...
	try:
		...
		print
		"[EXCEPTION] while deducing alpha from file_snapshot_corrected: ", e
		print
		" file_snapshot_corrected:",
		print
		' formula: int(file_snapshot_corrected.split("ALP-")[1].split(".")[0])'
		print
		""
		sys.exit(-1)
	...
	try:
		...
		print
		"[EXCEPTION] while deducing failure_probability from file_snapshot_corrected: ", e
		print
		" file_snapshot_corrected:",
		print
		' formula: int(file_snapshot_corrected.split("FAIL-")[1].split("-")[0])'
		print
		""
		sys.exit(-1)
	...
	out += " %d" % missing_snaps							#4
	out += " %d" % modified_snaps							#5
	out += " %d" % true_positives							#6
	out += " %d" % false_positives							#7
	out += " %d" % false_negatives							#8
	out += " %d" % true_negatives							#9
	...
	#Number of modified snapshots (?)

	#Number of accurately corrected snapshots, or true positives (?)
	#Number of false positive modifications (?)
	# Number of missing positive snapshots before processing (?)
	# out += "\\setf{%s} & " % format_number(true_positives)
	# out += "\\setf{%s} & " % format_number(modified_snaps-true_positives)
	# out += "\\setf{%s} & " % format_number(missing_snaps)

	# Suppose a computer program for recognizing dogs in photographs identifies
	# 8 dogs in a picture containing 12 dogs and 4 cats. Of the 8 identified as dogs,
	...
	a = true_positives + true_negatives
	b = true_positives + false_positives + false_negatives + true_negatives
	accuracy = None
	if b > 0:
		accuracy = 1.0 * a / b
		out += "&  %.02f  " % accuracy
	...
	# Precision = TP / TP + FP
	a = true_positives
	b = true_positives + false_positives #modified
	precision = None
	if b > 0:
		precision = 1.0 * a/b
		out += "&  %.02f  " % precision
	...
	# Recall = TP/TP+FN
	a = true_positives
	b = true_positives + false_negatives  # missing
	recall = None
	if b > 0:
		recall = 1.0 * a / b
		out += "&  %.02f  " % recall
